#!/usr/bin/env python3
# coding: utf-8
"""Evaluate pickplace next-action planning QA with VLM video models.
...
def video_paths_for_item(item: dict[str, Any]) -> list[Path]:
    input_data = item.get("input", {})
    clip_paths = input_data.get("clip_paths")
    if isinstance(clip_paths, list) and clip_paths:
        return [Path(str(path)) for path in clip_paths]

    clip_path = input_data.get("clip_path")
    if clip_path:
        return [Path(str(clip_path))]
    ...
    if isinstance(video_paths, dict) and video_paths:
        return [Path(str(path)) for path in video_paths.values()]

    video_path = input_data.get("video_path")
    if video_path:
        return [Path(str(video_path))]

    raise KeyError("planning item input must contain clip_path/clip_paths or video_path/video_paths")
...
def main() -> int:
    ...
    for index, item in enumerate(pending, 1):
        ...
        video_paths = video_paths_for_item(item)
